...
namespace Framework
{
	public class Scheduler<T> : SingletonMonoBehaviour<T> where T : Scheduler<T>
	{
		...
		public bool AddScheduler(SchedulerHandler handler,float delay,int times = 0)
		{
			...
			SchedulerEntity entity = ObjectPool<SchedulerEntity>.Instance.GetObject ();
		...
		private bool HasScheduler(SchedulerHandler handler)
		{
			bool result = false;
			for (int i = 0; i < _listSchedulerEntity.Count; i++)
			{
				...
				if (entity.state == SchedulerEntityState.ToDoAction)
				{
					try
					{
						if (!entity.OnTick (dt))
						{
							...
							SchedulerEntity RemoveEntity = ObjectPool<SchedulerEntity>.Instance.GetObject ();
